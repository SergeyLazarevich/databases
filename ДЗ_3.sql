...
) COMMENT 'Таблица сообществ для user';
...
) COMMENT 'Таблица типов медиаконтента';
...
) COMMENT 'Таблица медиаконтента для user';
...
CREATE TABLE Forum (
	...
	created_at DATETIME DEFAULT NOW(),
    updated_at DATETIME ON UPDATE CURRENT_TIMESTAMP,
	INDEX communities_name_idx(Topic),
	FOREIGN KEY (admin_user_id) REFERENCES users(id)
) COMMENT 'Таблица Форума'; 
...
CREATE TABLE forum_topic (
	...
	likes_forum_topic BIGINT UNSIGNED NOT NULL,
	topic_Text MEDIUMTEXT, 
...
) COMMENT 'Таблица Сататей'; 

ALTER TABLE vk.likes 
ADD CONSTRAINT likes_fk_2  
FOREIGN KEY (forum_topic_id) REFERENCES vk.forum_topic(id);
